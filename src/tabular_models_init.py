...
class StandardMLP(nn.Module):
    ...
    Main Backbone Network Architecture:
        [Linear -> ParametricActivation -> Dropout] x 3 -> Linear -> Logits
...
class NTLinear(nn.Module):
    ...
    Note on initialization:
        The RealMLP paper uses a data-dependent initialization.
...
class RealMLP_TD(nn.Module):
    ...
    def _forward_to_mlp_input(self, x_num, x_cat=None,record=True):
